...
function convertToRomanNumerals(num) {
  if (num > 39 || num < 1) throw new Error('Not valid number');
  let result = '';
  let remaining = num;
  while (remaining >= 10) {
    result += 'X';
...
function shuffleChar(str, iterations) {
  if (!str || iterations === 0) return str;
  let result = str;
  const len = str.length;
  let cycle = 1;
  function performShuffle(givenStr) {
    let even = '';
    let odd = '';
    for (let i = 0; i < givenStr.length; i += 1) {
      if (i % 2 === 0) {
        even += givenStr[i];
      } else {
        odd += givenStr[i];
  ...
  let temp = performShuffle(str);
  let iter = iterations;
  while (temp !== str && cycle <= len) {
    temp = performShuffle(temp);
    cycle += 1;
  }
  if (cycle < iter) {
    iter %= cycle;
